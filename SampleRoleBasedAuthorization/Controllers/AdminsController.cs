...
namespace SampleRoleBasedAuthorization.Controllers
{
    ...
    {
        role_based_authorizationContext _context = new role_based_authorizationContext();
        ...
        [HttpPost]
        public async Task<IActionResult> Create([Bind("Id,FullName,Email,Password,RolesId")] Admin admins)
        {
        ...
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        public async Task<IActionResult> Edit(int id, [Bind("Id,FullName,Email,RolesId")] Admin admins)
        {
            if (id != admins.Id)
            {
                return NotFound();
            ...
            Admin admin = await _context.Admins.Where(s => s.Id == admins.Id).FirstOrDefaultAsync();
